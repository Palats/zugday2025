// Logic to generate static data for the app. Called from CLI only.
// This is separated from binary entry point to help with type checking.

import { promises as fs } from "fs";
import * as csv from "csv-parse/sync";
import JSZip from "jszip";
import * as datatypes from "./datatypes";
import * as similarity from "./vendor/similarity";
...
export async function run() {
    ...
    for (const r of records) {
        ...
        allNames.add(r.designationOfficial);
    ...
    for (const m of missing) {
        const similar = similarity.findBestMatch(m, Array.from(allNames), 3);
        console.error(`Missing: ${m}; maybe one of ${similar.join("; ")} ?`);
    }
